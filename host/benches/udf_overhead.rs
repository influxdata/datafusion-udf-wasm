...
"
        ...
        let mut array_gen: Box<dyn FnMut() -> ArrayRef> = match payload {
            ...
            Payload::SubStr => {
                let mut input_gen = (0usize..).map(|x| {
                    // maximum length of sub strings
                    //
                    // Use a prime number around 100.
                    const MAX_LEN: usize = 97;
                    let s = std::iter::repeat_n('x', x % MAX_LEN).collect::<String>();
                ...
                Box::new(move || {
                    Arc::new(StringArray::from_iter((&mut input_gen).take(batch_size)))
                })
            }
        };
