...
/// [`PATHS`], but nicely printable.
fn paths_nice() -> Vec<String> {
    PATHS.iter().map(|s| s.replace("\0", r#"\0"#)).collect()
}
...
async fn run_1(udf: &WasmScalarUdf) -> String {
    ...
    let paths_nice = paths_nice();
    let mut out = String::new();
    let longest_path = paths_nice.iter().map(|p| p.len()).max().unwrap();
    for (path, res) in paths_nice.iter().zip(array) {
        let res = res.unwrap();
...
async fn run_2(udf: &WasmScalarUdf) -> String {
    ...
    let paths_nice = paths_nice();
    let (paths_nice_a, paths_nice_b) = cross(&paths_nice);
    let mut out = String::new();
    let longest_path = paths_nice.iter().map(|p| p.len()).max().unwrap();
    for ((path_a, path_b), res) in paths_nice_a.iter().zip(paths_nice_b).zip(array) {
        let res = res.unwrap();
