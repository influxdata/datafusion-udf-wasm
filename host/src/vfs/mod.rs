        ...
        if open_flags.contains(OpenFlags::TRUNCATE) && flags.contains(DescriptorFlags::WRITE) {
            ...
            match &mut node_guard.kind {
                VfsNodeKind::File { content } => {
                    ...
                    let node_attributes = node_guard.stat();

                    if node_attributes.type_ == DescriptorType::RegularFile {
                        // For regular files, `node_attributes.size` refers to
                        // the file size in bytes. For symbolic links, the
                        // length in bytes of the pathname contained in the
                        // symbolic link. We don't support symlinks, so we
                        // only handle regular files here.
                        //
                        // https://docs.rs/wasi-preview2-prototype/latest/wasi_preview2_prototype/snapshots/preview_2/wasi/filesystem/types/struct.DescriptorStat.html#structfield.size
                        let size = node_attributes.size;
                        self.vfs_state.storage_allocation.dec(size)?;
                    }
                }
