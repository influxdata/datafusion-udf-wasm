        ...
        for entry in archive.entries()? {
            ...
            let kind = match entry_type {
                ...
                tar::EntryType::Regular => {
                    // Enforce file size and total storage limits for seeded content, otherwise
                    // TAR population could exceed configured quotas.
                    ...
                    let current_storage = self.storage_allocation.get();
                    if current_storage.saturating_add(expected_size) > self.limits.max_storage_bytes
                    {
                        return Err(LimitExceeded {
                            name: "storage",
                    ...
                    self.storage_allocation.inc(content.len() as u64)?;
...
mod tests {
    use super::*;
    use crate::limiter::StaticResourceLimits;
    ...
    #[test]
    fn populate_from_tar_counts_seeded_storage_against_quota() {
        ...
        header.set_cksum();
        tar_builder
            .append_data(&mut header, "seed.txt", std::io::Cursor::new(data))
            .expect("append tar entry");
        let tar_data = tar_builder.into_inner().expect("finish tar");

        let pool: Arc<dyn MemoryPool> = Arc::new(UnboundedMemoryPool::default());
        let mut limiter = Limiter::new(StaticResourceLimits::default(), &pool);

        let err = vfs_state
            .populate_from_tar(&tar_data, &mut limiter)
            .unwrap_err();
        assert_eq!(err.kind(), std::io::ErrorKind::QuotaExceeded);
        assert_eq!(vfs_state.storage_allocation.get(), 0);
    }
    ...
    #[tokio::test]
    async fn truncate_is_rate_limited() {
        use filesystem::types::HostDescriptor;
        ...
        // Create a file (not rate-limited).
        let root_descriptor = create_rw_root_descriptor(&mut vfs_ctx);
        vfs_ctx
            .open_at(
                root_descriptor,
                PathFlags::empty(),
                "truncate_rate_limit.txt".to_string(),
                OpenFlags::CREATE,
                DescriptorFlags::READ | DescriptorFlags::WRITE,
            )
            .await
        ...
        // Truncate once: should consume the single allowed write-op in the 1s window.
        let root_descriptor = create_rw_root_descriptor(&mut vfs_ctx);
        vfs_ctx
            .open_at(
                root_descriptor,
        ...
        // If TRUNCATE is correctly rate-limited, we should have exhausted the budget.
        assert!(
            vfs_ctx.vfs_state.check_write_allowed().is_err(),
            "truncate should count against max_write_ops_per_sec",
        );
    }
